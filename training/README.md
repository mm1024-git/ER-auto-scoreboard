...
둘째는 기존 값을 변환하여 사용하는 방법이다. `app/configs/config-full.json`은 1080p 전체 화면에서 최대한 정밀하게 지정된 비율이므로, 본인 화면의 `game_area` 위치만 재계산한 뒤 동일한 변환을 `slots`의 각 값에 일괄 적용하는 편이 직접 지정하는 것보다 정확하다. `game_area`만 수정한 뒤 `slots` 변환을 LLM에게 요청하는 방식도 가능하다.
